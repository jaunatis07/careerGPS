...
export function buildResumeAnalysisSystemPrompt(
  ...
  return [
    ...
    "- 不确定的信息用「待核实」标注",
    "- 必须使用下方「输出格式」中的 Markdown 标题，便于用户阅读",
    "- jd_and_resume 模式下必须在「核心结论」中给出 Match 分数（0-100）",
    "",
    "# 输出格式（严格遵守）",
    "## 核心结论",
    "（一句话总结 + 关键数字，如 Match 分数）",
    "",
    "## 排雷要点",
    "（JD/企业/表述风险，条目列出）",
    "",
    "## 深度分析",
    "（按当前模式：JD 拆解 / 简历诊断 / 匹配 Gap 分析）",
    "",
    "## 修改与行动建议",
    "（Markdown 分模块给出可执行的简历改写或准备建议）",
  ]
    .filter(Boolean)
    .join("\n");
}
