...
- [ ] **2.2 数据库建表 (Database Schema)**  
  * 任务：在 Supabase SQL Editor 中运行 `ARCHITECTURE.md` 中的 SQL 脚本，创建 `user_profiles`、`chat_sessions`、`chat_messages` 四张表。  
  * 验证：Supabase Table Editor 页面成功看到这 4 张表。

- [ ] **2.3 登录/注册页面 UI 与逻辑 (`/login`)**  
